...
else
{
    while($row=$result->fetch_assoc())
    {
        if($row['RollNo']==$rno)
        {
            ...
            echo "</table></div></div>";
            echo "<a href='updatePage.php' style='background-color:#95eded; border-radius:10px; padding: 10px; border-width:0px; width:70px;'>Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;";
...
if($t==0)
{
    echo "<script>No Results found</script>";
    include("viewOneRecord.html");
}
$result->free();
$conn->close();
?>